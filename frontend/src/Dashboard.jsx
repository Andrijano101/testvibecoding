...
const PATTERN_LABELS = {
  ...
  value_just_below_threshold: "Nameštanje vrednosti ispod praga",
  zero_competition_repeat: "Višestruki ugovori bez konkurencije",
...
const PATTERN_EXPLANATIONS = {
  ...
  value_just_below_threshold: {
    icon: "🎯",
    title: "Nameštanje vrednosti ispod praga",
    why: "Firma prima više ugovora čija vrednost je sistematski postavljena tik ispod zakonskih pragova za sprovođenje javnog tendera. Istraživanje Transparentnosti Srbije (2024) pokazuje da 31,27% nabavki pada u zonu 900K–1M RSD — statistički nemoguće bez nameštanja.\n\nDokumentovani slučaj: Firma 'Avenija sistem' — 5 ugovora od 5 škola/fakulteta, svi između 2.948.000 i 2.999.250 RSD (tik ispod 3M praga za radove). Državni sekretar uhapšen.\n\nPragovi:\n• 1.000.000 RSD — ispod ovoga direktni sporazum\n• 3.000.000 RSD — za radove\n• 5.000.000 RSD — pojednostavljeni postupak\n• 15.000.000 RSD — puni tender",
    how: "(Institucija)-[AWARDED_CONTRACT]->(Ugovor)\n(Firma)-[WON_CONTRACT]->(Ugovor)\ngde: value_rsd IN [850K–1M, 2.7M–3M, 4.25M–5M, 12.75M–15M]\ncount >= 2 za isti par",
    sourcePortals: ["procurement"],
    sources: ["Portal javnih nabavki — vrednosti ugovora i naručioci"],
    fields: [
      { key: "company_name", label: "Firma" },
      { key: "institution", label: "Naručilac" },
      { key: "num_contracts", label: "Br. sumnjivih ugovora" },
      { key: "total_value", label: "Ukupna vrednost" },
    ],
  },
  zero_competition_repeat: {
    icon: "🚫",
    title: "Višestruki ugovori bez konkurencije",
    why: "Ista institucija dodeljuje isti firmi više ugovora putem pregovaračkog postupka bez objavljivanja (tip 3) ili direktnog sporazuma (tip 9). Srpski ZJN dozvoljava ove izuzetke samo u posebnim okolnostima — ako se 'izuzetak' ponavlja uvek sa istom firmom, radi se o zloupotrebi.",
    how: "(Institucija)-[AWARDED_CONTRACT]->(Ugovor)\n(Firma)-[WON_CONTRACT]->(Ugovor)\ngde: proc_type IN ['3','9'] ILI num_bidders = 1\ncount >= 2 za isti par Institucija–Firma",
    sourcePortals: ["procurement"],
    sources: ["Portal javnih nabavki — vrsta postupka i broj ponuđača"],
    fields: [
      { key: "institution", label: "Institucija" },
      { key: "company_name", label: "Firma" },
      { key: "num_contracts", label: "Br. bez-konkurencije ugovora" },
      { key: "total_value", label: "Ukupna vrednost" },
    ],
  },
  distributed_evasion: {
    icon: "🕸",
    title: "Raspoređeno izbegavanje nadzora",
    why: "Firma osvaja male ugovore od velikog broja različitih institucija. Svaki pojedinacan ugovor je ispod praga koji privlači pažnju, ali zbir svih je značajan. Umesto jednog vidljivog velikog ugovora, firma koristi mrežu malih ugovora koji se gube u sistemu.",
    how: "(Firma)-[WON_CONTRACT]->(Ugovor1, Ugovor2...)\n(Institucija1, Institucija2...)-[AWARDED_CONTRACT]->(Ugovori)\ncount(DISTINCT institucija) >= 3\nprosek po instituciji < 40% ukupno",
    sourcePortals: ["procurement"],
    sources: ["Portal javnih nabavki — istorija svih ugovora firme"],
    fields: [
      { key: "company_name", label: "Firma" },
      { key: "num_institutions", label: "Broj institucija" },
      { key: "total_contracts", label: "Ukupno ugovora" },
      { key: "total_value", label: "Ukupna vrednost" },
      { key: "avg_per_institution", label: "Prosek po instituciji" },
    ],
  },
  party_linked_contractor: {
    icon: "🎖",
    title: "Stranački kontakti — ugovor",
    why: "Narodni poslanik ili stranačko lice rukovodi firmom koja dobija ugovor od institucije gde je zaposlen drugi član iste stranke. Oba kraja lanca kontrolisana su od strane iste stranke — 'stranački zarobljeni tender'.",
    how: "(MP/Član stranke A)-[OWNS|DIRECTS]->(Firma)\n(Firma)-[WON_CONTRACT]->(Ugovor)\n(Institucija)-[AWARDED_CONTRACT]->(Ugovor)\n(Funkcioner stranke A)-[EMPLOYED_BY]->(Institucija)",
    sourcePortals: ["parliament", "apr", "procurement"],
    sources: ["Otvoreni Parlament — stranačka pripadnost", "APR — direktorska mesta", "Portal javnih nabavki"],
    fields: [
      { key: "mp_name", label: "Poslanik/Član stranke" },
      { key: "party_name", label: "Stranka" },
      { key: "company_name", label: "Firma" },
      { key: "contract_title", label: "Ugovor" },
      { key: "contract_value", label: "Vrednost" },
      { key: "institution", label: "Naručilac" },
      { key: "party_official_at_institution", label: "Stranački funkcioner u instituciji" },
    ],
  },
  suspicious_price_concentration: {
    icon: "📊",
    title: "Sektorski monopol",
    why: "Jedna firma osvaja 60%+ svih ugovora u svom sektoru delatnosti kod jedne institucije. U zdravom tržištu, različite firme iste delatnosti trebalo bi da ravnomerno učestvuju. Dominacija jedne firme ukazuje da je tender pisan po meri te firme.",
    how: "(Firma)-[:WON_CONTRACT]->(Ugovor)\n(Institucija)-[:AWARDED_CONTRACT]->(Ugovor)\nGROUP BY APR delatnost\nfirma_vrednost / ukupno_sektor >= 60%\nUslov: >= 2 konkurenta u sektoru",
    sourcePortals: ["apr", "procurement"],
    sources: ["APR — šifra delatnosti firme", "Portal javnih nabavki — ugovori po sektoru"],
    fields: [
      { key: "company_name", label: "Firma" },
      { key: "institution", label: "Institucija" },
